---
title: Commands
description: Complete command reference for BK-Gradients.
---

# Commands

All commands require the player to be online unless stated otherwise.
Admin commands can also be run from the console.

---

## `/bkgradients`

**Aliases:** `/bkg` · `/bkgrad` · `/gradients`
...
| `/prefix reset <player>` | Reset another player's prefix | `bkgradients.admin` |
...
| `/nick reset <player>` | Reset another player's nick | `bkgradients.admin` |
...
Same subcommands as `/prefix` — replace `prefix` with `suffix` in every command and permission node.
...
:::note[Tab completion]
All commands support tab completion for subcommands. Admin commands also tab-complete online player names.
:::
